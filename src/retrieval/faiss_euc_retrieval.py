...
def retrieve_needles_faiss_euc(haystack, needle, top_k=3, window_size=3):
    """
    Retrieve top-k passages from haystack relevant to the needle using FAISS (Euclidean distance).
    ...
    needle_emb = model.encode([needle], convert_to_numpy=True)
    distances, indices = index.search(needle_emb, top_k)
    top_passages = [passages[i] for i in indices[0]]
    return top_passages

def evaluate_faiss_euc_on_mimic(haystack_csv, top_k=3, window_size=3):
    """
    Evaluate FAISS retrieval on mimic_haystack.csv using Euclidean distance.
...
if __name__ == "__main__":
    ...
    results_df = evaluate_faiss_euc_on_mimic(haystack_file, top_k=2, window_size=3)

    output_dir = os.path.join("src", "retrieval", "outputs")
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, "faiss_euc_mimic_results.csv")
